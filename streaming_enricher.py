#!/usr/bin/env python3
"""Streaming enricher with adaptive chunking and retry logic.

This module processes prospect data in chunks, calling the LLM API to classify
job titles into personas. It implements adaptive chunk sizing, rate limit handling,
and multi-pass processing to maximize success rate.
...
def main(input_file_path: str) -> None:
    """Main processing function for streaming enrichment.

    Loads prospect data, processes it in adaptive chunks across multiple passes,
    and saves the results. Implements rate limiting, retry logic, and error handling.
    ...
    # Filter out test emails and rows without job titles
        ...
        with tqdm(total=len(df_pass)) as bar:
            while i < len(df_pass):
                ...
                try:
                    ...
                    # Track failed IDs for retry in next pass
